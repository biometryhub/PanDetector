...
while getopts g:d:n:o:m:c:a: option
do 
    case "${option}"
        in
        g)genome=${OPTARG};;
...
if [ "$1" == "-h" ]; then
  echo ""
  echo "PanDetector pipeline: for further help see https://github.com/biometryhub/PanDetector/"
  echo ""
  ...
  echo -e "Optional:\n\
	ncpus\t\tdefault is 4 cpus\n\
        ...
        accessory(whether or not generate accessory multiple sequence alignments.)\t\tdefault is 1 or enabled, to disable set it to 0\n\ 
...
java -jar -Xmx${mem}g MFbio.jar --task maf2msa --srcdir $outdir/filtered_maf --p1 ${maflist} --destdir $outdir/mfasta --file1 $outdir/msa.maf.gz --file2 $genome  --p2 ${accessory} --threads ${cores};
